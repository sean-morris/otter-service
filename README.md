...
| `release.yml` | Push of a version tag (`X.Y.Z`) or manual `workflow_dispatch` with optional `quick_test` | **Comprehensive grading validation before release:** Builds Docker image, fetches ALL notebooks from all courses (88ex, 88bx, 88cx, 8x), validates student notebooks fail and solution notebooks pass. **If tests pass:** creates GitHub release with changelog, publishes to PyPI, builds and pushes Docker image via Cloud Build, opens PR in [edx-hub](https://github.com/edx-berkeley/edx-hub) to update `otter-srv` image tag. **If tests fail:** stops and does not publish. Optional `quick_test: true` input skips full test and runs lab01 only (~10 min vs ~60 min) for hotfix deployments. |
